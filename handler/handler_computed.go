...
func TriggerComputed(ctx context.Context, redisClient redisdb.Client, mongoClient *mongo.Client, mq mq.MQ, apiClient api.Client, projectName string, data entity.DataMessage) error {
	...
	err := event.GetByType(ctx, redisClient, mongoClient, projectName, string(ComputeLogic), eventInfoList)
	if err != nil {
