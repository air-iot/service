...
type (
	Subscriber chan entity.MQRealTimeData
	TopicFunc  func(entity.MQRealTimeData) bool
)
...
func (p *Publisher) Publish(v entity.MQRealTimeData) {
	p.m.RLock()
	defer p.m.RUnlock()
...
func (p *Publisher) sendTopic(sub Subscriber, topic TopicFunc, v entity.MQRealTimeData, wg *sync.WaitGroup) {
	defer wg.Done()
