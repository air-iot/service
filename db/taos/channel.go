...
type Config struct {
	...
	//关闭连接的方法
	Close func(*sqlx.DB) error
	//检查连接是否有效的方法
	Ping func(*sqlx.DB) error
	//连接最大空闲时间，超过该事件则将失效
...
// Get 从pool中取一个连接
func (c *channelPool) Get() (*sqlx.DB, error) {
	conns := c.getConns()
	if conns == nil {
...
// Put 将连接放回pool中
func (c *channelPool) Put(conn *sqlx.DB) error {
	if conn == nil {
...
// Close 关闭单条连接
func (c *channelPool) Close(conn *sqlx.DB) error {
	if conn == nil {
		return errors.New("connection is nil. rejecting")
	}
	c.mu.Lock()
...
// Ping 检查单条连接是否有效
func (c *channelPool) Ping(conn *sqlx.DB) error {
	if conn == nil {
		return errors.New("connection is nil. rejecting")
	}
	return c.ping(conn)
