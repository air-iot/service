...
func TriggerComputedFlow(ctx context.Context, redisClient redisdb.Client, mongoClient *mongo.Client, mq mq.MQ, apiClient api.Client, zbClient zbc.Client, projectName string, data entity.DataMessage) error {
	...
	for _, flowInfo := range *flowInfoList {
		...
		if invalid, ok := settings["invalid"].(bool); ok {
			if invalid {
				logger.Warnln(flowComputeLogicLog, "流程(%s)已经失效", flowID)
				continue
			}
		}
		...
		if disable, ok := settings["disable"].(bool); ok {
			if disable {
				logger.Warnln(flowComputeLogicLog, "流程(%s)已经被禁用", flowID)
				continue
			}
		}
